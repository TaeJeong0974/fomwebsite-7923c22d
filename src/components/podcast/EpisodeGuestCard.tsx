...
const EpisodeGuestCard = ({ name, title, company, companyDomain, linkedInUrl, youtubeUrl, spotifyUrl }: EpisodeGuestCardProps) => {
    ...
    <SidebarCard title="Guest">
      <div>
        ...
        <div className="flex items-center gap-2 mt-3">
          {linkedInUrl && (
            <a href={linkedInUrl} target="_blank" rel="noopener noreferrer" className={iconBtnClass} aria-label={`${name} on LinkedIn`}>
              <Linkedin size={14} />
            </a>
          )}
          {companyUrl && (
            <a href={companyUrl} target="_blank" rel="noopener noreferrer" className={iconBtnClass} aria-label={`${company} website`}>
              <Building2 size={14} />
            </a>
          )}
          {youtubeUrl && (
