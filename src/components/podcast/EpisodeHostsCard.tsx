...
const HostCard = ({ host, isFirst }: { host: typeof podcastHosts[0]; isFirst: boolean }) => {
  const [isHovered, setIsHovered] = useState(false);
  
  return (
    <div 
      className="relative glass rounded-xl p-5 sm:p-6 overflow-hidden min-w-[calc(80%-8px)] sm:min-w-0 flex-shrink-0 sm:flex-shrink"
...
const EpisodeHostsCard = ({ showAllHosts = false }: EpisodeHostsCardProps) => {
  ...
  if (isMobile) {
    return (
      <div className="space-y-4">
        ...
        <div 
          ref={scrollRef}
          className="flex gap-3 overflow-x-auto scrollbar-hide -mr-6 pr-6"
          style={{ scrollSnapType: 'x mandatory' }}
        >
          {hosts.map((host, index) => (
            <div key={index} style={{ scrollSnapAlign: 'start' }}>
              <HostCard host={host} isFirst={index === 0} />
            </div>
          ))}
        </div>
      </div>
  ...
  // Desktop: stacked layout with single card
  return (
    <div 
