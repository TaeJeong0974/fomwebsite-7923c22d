              ...
              <tr key={s.id} className="border-b border-black/20 last:border-0 hover:bg-black/5">
                <td className="px-2 py-2 text-[11px] font-bold" style={macFont}>
                  <div className="flex items-center gap-2">
                    {s.image_url ? (
                      <img src={s.image_url} alt={s.name} className="h-7 w-7 rounded-full object-cover border border-black shrink-0" />
                    ) : (
                      <div className="h-7 w-7 rounded-full bg-black/10 border border-black/20 shrink-0 flex items-center justify-center text-[9px] font-bold" style={macFont}>
                        {s.name.charAt(0)}
                      </div>
                    )}
                    {s.name}
