...
const PodcastSection = () => {
  ...
  const themes = useMemo(() => {
    ...
    for (const ep of publishedEpisodes) {
      for (const t of ep.themes ?? []) set.add(t);
    }
  ...
  const sortedPublished = useMemo(
    () => sortEpisodes(filteredPublished, sortMode),
    [filteredPublished, sortMode]
  );
  const sortedComingSoon = useMemo(
    () => sortEpisodes(filteredComingSoon, sortMode),
    [filteredComingSoon, sortMode]
  );

  const activeLabel =
    SORT_OPTIONS.find((o) => o.value === sortMode)?.label ?? "Newest First";
  ...
  return (
    <section
      id="podcast"
      className="pt-16 md:pt-20 lg:pt-24 pb-14 md:pb-16 lg:pb-20 scroll-mt-24 md:scroll-mt-28"
    >
      <div className="container mx-auto container-padding">
        {/* Header */}
        <div className="flex items-end justify-between gap-4 sm:gap-6 mb-8 md:mb-10 lg:mb-12">
          <div>
            <p className="text-label font-medium text-foreground mb-2 sm:mb-3">
              EPISODES
            </p>
            <h2 className="text-display-xl font-medium text-foreground">
              Podcast
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div ref={sortRef} className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`h-3.5 w-3.5 shrink-0 transition-transform duration-300 ${sortOpen ? "rotate-180" : ""}`}
                />
              </button>
            ...
            <div className="glass rounded-full p-1.5 flex items-center gap-1 !shadow-none hover:!shadow-glass transition-shadow duration-300 relative h-[42px]">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium pr-1 capitalize leading-none translate-y-[1px] relative z-10">
                    {type}
                  </span>
                </button>
              ))}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {layout === "grid" ? (
              <PodcastGridView
                episodes={sortedPublished}
                comingSoonEpisodes={sortedComingSoon}
              />
            ) : (
              <PodcastListView
                episodes={sortedPublished}
