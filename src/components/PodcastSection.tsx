      ...
      {hasMore && (
        ...
        >
          <LiquidButton
            ...
            className="bg-white/40 hover:bg-white/60 shadow-[0_2px_12px_-2px_rgba(0,0,0,0.1),inset_0_1px_0_rgba(255,255,255,0.8)] hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.15),inset_0_1px_0_rgba(255,255,255,1)] text-center"
          >
            <span className="w-full text-center">Load More</span>
          </LiquidButton>
        </motion.div>
      )}
    </div>
                ...
                <div className="flex items-start gap-4 sm:gap-6 lg:gap-10">
                  <span className={`text-label pt-0 sm:pt-2 transition-opacity duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${!isMobile && hoveredIndex !== null && hoveredIndex !== index ? 'opacity-30' : ''}`}>EP {String(index + 1).padStart(2, '0')}</span>
                    ...
                    <div className="flex items-start gap-4 lg:gap-6">
                      ...
                      <p className={`hidden lg:block text-body-sm pt-1 text-foreground/60 transition-opacity duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${!isMobile && hoveredIndex !== null && hoveredIndex !== index ? 'opacity-30' : ''}`}>
                    ...
                    <p className={`lg:hidden mt-2 text-body-sm text-foreground/60 transition-opacity duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${!isMobile && hoveredIndex !== null && hoveredIndex !== index ? 'opacity-30' : ''}`}>
                      {isIntroEpisode ? podcastHosts.map((h, i) => <span key={h.name}>{h.name}{i < podcastHosts.length - 1 && ', '}</span>) : <>{episode.title} <span className="font-medium">@ {episode.company}</span></>}
                    </p>
                  </div>
                </div>
